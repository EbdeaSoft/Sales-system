...
<td>
                                            <a class="modal-effect btn btn-sm btn-info" data-effect="effect-scale"
                                                data-branchid="{{ $branch->id }}" data-branchname="{{ $branch->name }}"
                                                data-branchplace="{{ $branch->place }}"
                                                data-toggle="modal" href="#modaldemo8" title="{{__('roles.update')}}"><i
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content modal-content-demo">
                <div class="modal-header">
                    <h6 class="modal-title"> {{__('home.update_branch_data')}}</h6><button aria-label="Close" class="close"
                        data-dismiss="modal" type="button"><span aria-hidden="true">&times;</span></button>
                </div>
                <form action="{{ url(Mcamara\LaravelLocalization\Facades\LaravelLocalization::getCurrentLocale().'/'. ($page = 'updatebranch')) }}" method="post">
                    {{ csrf_field() }}
...
<input type="text" class="form-control" id="id"
                                ...
                                <input type="text" class="form-control" id="breanchName"
                                    name="breanchName" title="  يرجي ادخال رقم الفاتورة  " 
                                   required >
                                ...
                                <input type="text" class="form-control" id="branchLoction"
                                    name="branchLoction" title="  يرجي ادخال رقم الفاتورة  " 
                                   required >

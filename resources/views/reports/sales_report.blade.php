...
@if ($invoice->Pay == 'Cash')
    ...
    <td> {{__('home.total')}}</td>
    <td>{{($invoice->Added_Value+$invoice->Price)}}</td>
...
<a class="btn btn-success" href="{{ url('/' . ($page = 'printInvoicesReport').'/'.$pay[1].'/'.$pay[0].'/'.$startat.'/'.$endat) }}"> {{__('home.print')}}</a>
</div>
<br>
                    @endif
                </div>

...
@section('title')
{{__('report.Purchasesـfromـsuppliers')}}@stop
@endsection
@section('page-header')
<!-- breadcrumb -->
<div class="breadcrumb-header justify-content-between">
    <div class="my-auto">
        <div class="d-flex">
            <h4 class="content-title mb-0 my-auto">{{__('report.Purchasesـfromـsuppliers')}}
...
?>
    <tbody>
        ...
        <td>{{$product->productData->Product_Code}}</td>
            ...
            <td>{{($product->numberofpice*$product->Added_Value)+($product->numberofpice*$product->purchasingـprice)}}</td>
        </tr>
...
@endforeach
          ...
          <span class="text-warning  float-left mt-3 mr-2" id="print_Button" >{{__('home.total')}} : {{($invoice->In_debt)}}</span>
...
<a class="btn btn-success" href="{{ url('/' . ($page = 'print_Purchasesـfromـsuppliers').'/'.$pay[1].'/'.$pay[0].'/'.$startat.'/'.$endat) }}"> {{__('home.print')}}</a>
</div>
<br>
                    @endif
                </div>
